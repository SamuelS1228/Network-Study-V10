...
with st.sidebar:
    ...
    cost_per_lb_mile = st.number_input("Transportation cost $ per lb‑mile", value=0.02)
    fixed_cost = st.number_input("Fixed cost per warehouse ($/yr)", value=250000.0, step=50000.0)
    sqft_per_lb = st.number_input("Warehouse sqft per lb", value=0.02)
    cost_per_sqft = st.number_input("Variable cost $ per sqft per yr", value=6.0)
    st.markdown("---")
